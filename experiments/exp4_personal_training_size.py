...
# Experiment 4 – Personal – RandomForest: Vplyv veľkosti trénovacej množiny (TRAIN_SIZE) na výkon
...
TRAINING_DIR = os.path.join(os.path.dirname(__file__), "..", "data", "training_personal")
RESULTS_DIR  = os.path.join(os.path.dirname(__file__), "results", "exp4_personal_training_size_analysis")
RANDOM_STATE = 42
...
def plot_eer_vs_train_size(summary_df: pd.DataFrame, out_dir: str):
    ...
    ax.set_title("Experiment 4 Personal – EER vs počet tréningových vzoriek (RandomForest)")
    ax.set_ylabel("EER")
...
def plot_far_frr_vs_train_size(summary_df: pd.DataFrame, out_dir: str):
    ...
    ax.set_title("Experiment 4 Personal – FAR a FRR vs počet tréningových vzoriek (RandomForest)")
...
def main():
    print("=" * 80)
    print("Experiment 4 Personal – RandomForest: vplyv veľkosti trénovacej množiny (personal text)")
    ...
    print("\nExperiment 4 Personal dokončený.")
